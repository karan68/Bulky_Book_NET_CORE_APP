        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //as we are deploying will comment out these
                    //if(!await _roleManager.RoleExistsAsync(SD.Role_Admin)) //checking if the role exists in SD
                    //{
                    //    await _roleManager.CreateAsync(new IdentityRole(SD.Role_Admin)); //if not create one
                    //}
                    ...
                    if (user.Role == null) //if the user has no roll i.e. new user
                    {
                        await _userManager.AddToRoleAsync(user, SD.Role_User_Indi);
                    }
                    else
                    {
                        if (user.CompanyId > 0) //if it is sa company 
                        {
                            await _userManager.AddToRoleAsync(user, SD.Role_User_Comp);
                        }
                        await _userManager.AddToRoleAsync(user, user.Role); //if it has some other role
                    }
                    // await _userManager.AddToRoleAsync(user, SD.Role_Admin); //for now anyone who registers is a admin we will change it later
